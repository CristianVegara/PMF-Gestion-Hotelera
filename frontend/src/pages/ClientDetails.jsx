...
const ClientDetails = () => {
  ...
  const handleBookingClick = (book) => {
    if (
      selectedRange.start === book.fechaEntrada &&
      selectedRange.end === book.fechaSalida
    ) {
  ...
  const activeBookingWithRoom = bookings.find(
    b => b.checkInStatus === 'DENTRO' && b.habitacion?.id
  );
    ...
    <div className="client-details-page">
      ...
      <div className="details-section-card profile-main-card">
        ...
        <div className="profile-data">
          ...
          {activeBookingWithRoom && (
            <div className="active-booking-banner"
              onClick={() => navigate(`/rooms/${activeBookingWithRoom.habitacion.id}`)}>
              Actualmente alojado en habitación Nº {activeBookingWithRoom.habitacion.number}
            </div>
          )}
          ...
          <div className="details-table-wrapper">
            <table className="details-mini-table">
              ...
              <tbody>
                {bookings.map(book => {
                  const isHighlighted =
                    activeRange.start === book.fechaEntrada &&
                    activeRange.end === book.fechaSalida;
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={() =>
                        setHoverRange({
                          start: book.fechaEntrada,
                          end: book.fechaSalida
                        })
                      }
                      onMouseLeave={() =>
                        setHoverRange({ start: null, end: null })
                      }
                    ...
                    <tr
                      ...
                      onMouseEnter={() => {
                        const parentBooking = bookings.find(b =>
                          isDateInRange(actDate, {
                            start: b.fechaEntrada,
                            end: b.fechaSalida
                          })
                        );
                        ...
                        if (parentBooking) {
                          setHoverRange({
                            start: parentBooking.fechaEntrada,
                            end: parentBooking.fechaSalida
                          });
                        }
                      }}
                      onMouseLeave={() =>
