...
describe('Pruebas en el componente ClientsInHouse', () => {
  ...
  test('Mostrar la lista de huéspedes en casa y las estadísticas correctamente', async () => {
    ...
    const statHorizontal = screen.getByText('En Casa (Dentro)').previousSibling;
    expect(statHorizontal.textContent).toBe('1');

    const statCheckIns = screen.getByText('Entradas Hoy').previousSibling;
    expect(statCheckIns.textContent).toBe('1');
  });
  ...
  test('Filtrar la tabla cuando se busca por nombre de huésped', async () => {
    ...
    const searchInput = screen.getByPlaceholderText('Buscar huésped o habitación...');
    fireEvent.change(searchInput, { target: { value: 'Inexistente' } });

    expect(screen.queryByText('Carlos Mendoza')).not.toBeInTheDocument();
    expect(screen.getByText('No hay huéspedes "Dentro" registrados para hoy.')).toBeInTheDocument();
  });
  ...
  test('Manejar una lista vacía de huéspedes de manera correcta', async () => {
    ...
    expect(await screen.findByText('No hay huéspedes "Dentro" registrados para hoy.')).toBeInTheDocument();
  });
});